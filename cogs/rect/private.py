...
class spla3(View):
    ...
    async def callback_saibo(self, button: Button, interaction: discord.Interaction):
        await interaction.response.send_message(f"近日実装予定",ephemeral = True)
    #async def callback_saibo(self, button: Button, interaction: discord.Interaction):
        #embed = discord.Embed()
        #embed.add_field(name=f"{interaction.user.name}の再募集", value=f"{interaction.message.content}")
        #await interaction.response.send_message(f"<@&983297498271580170>",ephemeral = True,view=self,embed=embed)

class rectprivate(discord.ui.Modal):
...
class private(commands.Cog):
    ...
    @slash_command(name="募集プラベ",guild_ids=guild_ids, description="プライベートマッチメンバーの募集")
    async def rectprivate(self, interaction: discord.Interaction):
        if interaction.channel.id not in [981474117020712973, 1027901157076631582]:
            return await interaction.respond("エラー：プラベ募集コマンドは <#1027901157076631582> で実行して下さい。",ephemeral = True)
        modal = rectprivate(title="募集の詳細を説明")
        await interaction.response.send_modal(modal)

def setup(bot: commands.Bot):
    bot.add_cog(private(bot=bot))