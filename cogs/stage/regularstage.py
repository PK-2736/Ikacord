...
print("regularstageの読み込み完了")

class regularstage(commands.Cog):
    def __init__(self, bot):
        self.bot = bot
    
    @slash_command(name='レギュラー', guild_ids = guild_ids, description='レギュラーのステージ情報を取得します。')
    async def regular(self, ctx, time : Option(str, '時間', choices=["今", "次", "次の次"])):
        ...
        if time == "次":
            ...
            embed2.set_thumbnail(url="https://cdn.wikimg.net/en/splatoonwiki/images/4/4c/Mode_Icon_Regular_Battle_2.png")
        ...
        if time == "次の次":
            ...
            embed3.set_thumbnail(url="https://cdn.wikimg.net/en/splatoonwiki/images/4/4c/Mode_Icon_Regular_Battle_2.png")
            embed3.set_footer(text="API: https://spla3.yuu26.com| イカコード3")
    
            await ctx.respond(embed=embed3,file=file2)

def setup(bot):
    bot.add_cog(regularstage(bot))
